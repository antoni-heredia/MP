...
Matriz2D  CreaMatriz2D (int fils, int cols){
  ...
  matriz.datos[0] = new int [fils*cols];

  // Inicializar correctamente el vector de punteros,
  // desde las casillas 1 a la "fils"-1 (la casilla 0
  // ya está inicializada).
  // Apuntar con "datos[f]" al inicio de la fila "f"
  for(int f=1; f<fils ; f++)
    matriz.datos[f] = matriz.datos[f-1] + cols;
